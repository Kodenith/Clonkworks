...
public func Set(int iAngle, int iWidth, int iDist, int iT, object pDmg, object pAtt, int iReflect, object Parent1, object Parent2)
{
  // optional parameters
  if(!iWidth) iWidth = 3;
  if(!iDist) iDist = 300;
...
private func HitCheck() {
  ...
  while(++ml) {
    ...
    var pObs=FindObject2(Find_Distance(10,mx*ml/iMaxDist,my*ml/iMaxDist), 
                   Find_Exclude(this), 
                   Find_Or(Find_Func("IsBulletTarget"),Find_OCF(OCF_Alive)));
  ...
  var clonk;
  clonk = iParent1->~GetUser();
  if(!clonk) clonk = iParent1;
  var clonk2;
  clonk2 = iParent2->~GetUser();
  if(!clonk) clonk2 = iParent2;
	
	var temp_l, hitobject;
	
  // search points on the line for clonks etc.
  // step for step
  for(pObj in FindObjects(Find_Distance(22,mx,my), Sort_Distance(0,0),
              Find_Exclude(this()),
              Find_Exclude(pAttach),
              Find_NoContainer(),
              Find_Or(
              	Find_Func("IsBulletTarget",GetID(),this(),clonk),
              	Find_Func("IsBulletTarget",GetID(),this(),clonk2),
              	Find_OCF(OCF_Alive)),
              Find_Func("CheckEnemy",this())))
  {
    //Message(Format("Wizard1 %s | Wizard2 %s", GetName(clonk),GetName(clonk2)));
    //Die Verursacher des Starken Lasers werden durch ihren eigenen Strahl nicht getroffen
    if(pObj == clonk) continue;
    if(pObj == clonk2) continue;
  	...
  	if(LocalN("pDmgCallback",iParent1) ->~ LaserStrike(pObj,c))
  	if(LocalN("pDmgCallback",iParent2) ->~ LaserStrike(pObj,c))
  	{
  		SetLaser(0);
  		l = temp_l;
  		hitobject = true;
  ...
  if(pObstacle)
      {
         ...
         DoDamage(2,target);
         DoEnergy(-2,target,0,0,GetOwner(this)+1);
      }
      if(wallhit)
      {
      ...
      {
        CreateParticle("Terashock",x,y,0,0,MiningCharge*10,GetClrModulation()); 
        MiningCharge++; 
        if(MiningCharge>60) 
        {
         CreateObject(_BCR,x,y)-> Explode(50);
         //BlastFree(x+GetX(),y+GetY(), 50, GetOwner(this)+1);
         MiningCharge=0;
...
func CheckGround(int length)
{
	...
	//nichts gefunden? wir m ssen im Material feststecken.
