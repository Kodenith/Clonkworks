...
{
  var oreTotal = ObjectCount2(Find_ID(GOLD));
  for(var i in FindObjects(Find_ID(GDEP))){
	  oreTotal -= ContentsCount(GOLD,i);
  }
  var oreOutside = ObjectCount2(Find_ID(GOLD), Find_NoContainer());
  var deposits = ObjectCount(GDEP);
  var Units = Max(GetMaterialCount(Material("Gold")) - 50, 0) / 100;
