...
{
  // kein Men� bei Aktion in angefasstem Objekt
  if(GetAction(pByObj) == "Push" && GetActionTarget(0, pByObj)->~ControlDigDouble())
    return 0;
  // Clonk stoppen und Seilmen� erzeugen
  ...
  // Folgende Eintr�ge: Inventarobjekte
  ...
  // Auch Zweikampfgegner auff�hren
  ...
  // Wenn Gefangennahme-Regel aktiv: zuletzt gefangennehmbare Clonks auff�hren
  ...
  for(var pObj in FindObjects(Find_Or(Find_AtPoint(), Find_InRect(-10, -15, 20, 30)), Find_OCF(OCF_CrewMember), Find_NoContainer(), Find_Exclude(pByObj)))
  {
    ...
    // ...und wieder entfernen, damit es nur im Men� erscheint
...
{
  // Nochmal alle m�glichen Objekte durchgehen
  ...
  // Abbrechen, wenn Objekt nicht mehr da (au�er Zweikampfgegnern)
...
private func TieUp(idTo, pTo)
{
  ...
  {
    // 3. Parameter f�r OnStartTiedMember: Wenn 1, dann war der Clonk schon vorher gefesselt
    ...
    // Info den Szenarioscripten, Spielzielen usw. zur Verf�gung stellen
...
global func SetTiedPhysicals(pClonk)
{
  ...
  // Keine Hand frei zum Bauen oder F�llen
...
{
  // erster Vertex: Geschwindigkeit von 1.Actiontarget ber�cksichtigen
  if(i == 0) return Distance(
    GetPoint(0) + GetXDir(pTarget1) / 2, 
    GetPoint(0, 1) + GetYDir(pTarget1) / 2,
    GetPoint(1),
    GetPoint(1, 1));
  // vorletzter Vertex: Geschwindigkeit von 2.Actiontarget ber�cksichtigen
...
func MoveRope(pObj1, pObj2)
{
  ...
  for(var i = GetPointNum() - 2; i > 0; i--)
  {
    ...
    // �bersch�ssige Vertices, die nahe genug aneinander sind entfernen
    if(GetPointNum () > 3)
    ...
    // bei zu langem Abstand einen neuen Vertex einf�gen
    ...
    // Wenn durch verschieben des Vertex das Seil durch solides ginge, nicht verschieben, und bei genug Platz einen zus�tzlichen Vertex einf�gen
...
func SetRopeEnd(pObj, iRopeVtx, iObjVtx, iWhichEnd)
{
  ...
  // ginge, einen Vertex an der letzten Position des Objekts einf�gen
  // wenn das Objekt selbst in fester Materie steckt, h�lfe das auch nichts
...
func Connecting()
{
  // Wenn angekn�pfte Objekte verloren gehen: Seil l�schen
  ...
  // bei zu kurzer Seill�nge nicht mehr ziehen, um Aufschaukeln zu verhindern
  ...
  if(GetID(pTarget1) == CL5P && GetID(pTarget2) == CL5P && !Contained(pTarget1) && !Contained(pTarget2))
    return 0;
  // wenn eine Seilrolle Seil gibt, dann in Ruhe lassen, au�er, sie d�mpft
  // den Fall von Clonks
...
private func PullObject(iToX, iToY, iLength, pObj, pObj2, iVtx) // pObj wird versetzt
{
  ...
  // Au�nahmen:
  // gefesselte Clonks lassen sich abf�hren
  if(GetEffect("IntTied", pObj2))
    return 0;
  // Geb�ude (oder Inhaltsobjekte) sowie Hintergrundobjekte gar nicht ziehen
  if(GetCategory(pObj) & C4D_Structure || GetCategory(pObj) & C4D_StaticBack)
    return 0;
  // Objekt aus dem eigenen Grafikbereich heraus nicht ziehen, f�hrt nur zu komischen Effekten
  ...
  // Floateinfluss: z.B. Luftschiffe k�nnen heben
  ...
  //Falls Objekt dann feststecken w�rde, nicht wirklich versetzen
  if(Stuck(pObj))
    SetPosition(GetX(pObj) - BoundBy(x, -1, 1), GetY(pObj) + BoundBy(y, -1, 1), pObj);
 }
...
private func GetMiddlestVertex(pObj)
{
  ...
  {
    // x-Differenz zur Mitte + y-Differenz (eigentlich m�sste ich Distance benutzen)
...
// �ndert die angekn�pften Objekte
...
{
  // kein Men� bei angefasstem Objekt
