...
//kodenith - to balance out the cavecrawler, the blimp may not go on walls.
func WallCheck(){
	if(!GBackSolid() && !GBackLiquid() && GetMaterial() != Material("Sky")) SetPhysical("Float",30,2);
	else SetPhysical("Float",100,2);
}