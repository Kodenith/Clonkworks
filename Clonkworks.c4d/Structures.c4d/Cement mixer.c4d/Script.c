...
protected func Construction()
{
  ...
  if(!basement) basement = CreateObject(BasementID(), -5, 8, GetOwner() );
  return(_inherited() );
}

public func ContextProduction(pCaller)
{
  [$TxtProduce$|Image=C_MX|Condition=IsBuilt]
...
public func ProductCondition () { return(0); }
...
public func IsProducerOf (caller, def) {
  ...
  if (!def->~IsMixerProduct()) return (0);
  if (!IsBuilt ()) return (0);
  return(1);
}
...
public func ProductionStart(pWorker, idProduct)
{
  ...
  product = idProduct;
  // Clonk einsaugen
  pWorker->SetAction("Push");
  SetActionTargets(this(),0,pWorker);
  SetX(GetX(pWorker)-10,pWorker);
  SetDir(DIR_Right,pWorker);
  ...
  var dookie;
  while(dookie = FindObject2(Find_OCF(OCF_CrewMember), Find_Action("Push"), Find_ActionTarget(this()), Find_NoContainer(), Find_Exclude(pWorker))){
	  dookie->SetAction("Walk");
  }
...
private func Mix() {
  ...
  if(pProduct && FindCrewContents()) Enter(FindCrewContents(),pProduct);
  ProductionCancel();
  return(1);
}
...
private func ProductionCancel() {
  // Arbeit abbrechen
  if (GetAction() == "Mixing") SetAction("Idle");
  // Clonk(s) auswerfen
  var clnk;
  while (clnk = FindCrewContents()){
	  clnk->SetAction("Walk");
  }
  // Fertig
  return(1);
...
private func FindCrewContents() {
  var i = 0, obj;
  return(FindObject2(Find_OCF(OCF_CrewMember), Find_Action("Push"), Find_ActionTarget(this()), Find_NoContainer()));
}
...
private func CheckHolder(){
	if(!FindCrewContents()) ProductionCancel();
}

public func RejectGrabbed(){
	if(GetAction() == "Mixing") return(1);
}