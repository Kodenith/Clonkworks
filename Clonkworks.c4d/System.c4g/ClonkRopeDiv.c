...
protected func ControlDigDouble(pClonk)
{
  ...
  // Eklige Effekte bei Mischung mit Leitungsbaus�tzen... Finger weg!
  ...
  for(var pRope in FindObjects(Find_ID(CK5P)))
  {
    for(var pObj in FindObjects(Find_Or(Find_AtPoint(), Find_InRect(-10, -15, 20, 30)), Find_Not(Find_ID(CL5P)), Find_Not(Find_And(Find_AnyContainer(), Find_Not(Find_Container(this))))))
    {
      ...
      {
        // Bestimmte Seile k�nnen nicht abgenommen werden (z.B. Enterhaken, Anker)
        ...
        if(pObj == this && GetEffectCount("IntTied", this) && EffectVar(0, this, GetEffect("IntTied", this)) == pRope)
          return _inherited(pClonk, ...);
        Sound("ArrowHit");
        // Seilabnahme von Seilrolle w�rde zu zusammenf�gen des Seiles f�hren
...
{
  // War gefesselt? F�r Auswertung in OnClonkDeath Effekt noch mal wiederherstellen
  if(FindObject2(Find_ID(CY5P), Find_ActionTarget(this)))
    AddEffect("IntTied", this, 25);
  // Effekt erst NACH OnClonkDeath l�schen
...
/*--------------------------------------------------------*/
/*-- Enterhaken k�nnen auch im Springen geworfen werden --*/
/*--------------------------------------------------------*/
...
// Launch �ber Ejection im Clonk statt Departure im Enterhaken, so verschwinden Zugeffekte (?!)
func Ejection(pObj)
{
