...
{
  SetAction("Ready");
  // Nach rechts ausgerichtet, wenn in der linken Landschaftsh�lfte
...
public func SpellFailed(id idSpell, object pByClonk)
{
  ...
  // Alchemische Zutaten k�nnten aus dem Clonk oder dem Turm stammen: Dem Turm geben, wenn m�glich
  if(ObjectCount(ALCO))
...
public func SelectSpell(object caller, int direction)
{
  // Forschung abbrechen
  CancelResearch();
  // Men� erzeugen und mit den Zauberspr�chen des Turmbesitzers f�llen
  // Geh�rt der Turm keinem g�l�tigen Besitzer, stattdessen Zauber des Clonks nehmen
...
protected func ResearchMenu(idImage, pClonk) 
{
  ...
  // Alle geladenen Spr�che �berpr�fen
